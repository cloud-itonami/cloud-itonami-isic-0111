(ns cerealops.operation
  "OperationActor -- one cereal-growing operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (CerealOpsAdvisor) is sealed into a single node (:advise); its
  ...
  rewrite:
    - the Store    (MemStore | DatomicStore, see `cerealops.store`)
    - the Advisor  (mock today; real LLM is the next seam --
                     `cerealops.advisor/Advisor` is already the
                     injection point, see its docstring)
    - the Phase    (0->3 rollout)

  One graph run = one cereal-growing coordination operation. No
  ...
  `cerealops.store`'s append-only ledger (`store/append-ledger!`), so
  a field's full operating history is always a query over an
  immutable log.
  ...
  `cerealops.governor/always-escalate-ops`. Mirrors
  `marketentry.operation` (cloud-itonami-iso3166-ago) node/edge
  structure exactly, wired to this repo's own advisor/governor/phase/
  store."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [cerealops.advisor :as advisor]
...
(defn- commit-fact
  ...
  schedule, concern, supply order) -- cerealops has no separate
