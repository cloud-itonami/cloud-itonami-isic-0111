(ns cerealops.operation
  "OperationActor -- one cereal-growing operation = one supervised actor
  run. The advisor (CerealOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Field Operations
  ...
  One run = one cereal-growing coordination operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner loop
  -- each operation is auditable and checkpointed. A field's operating
  history is advanced by MANY operations (log-field-record /
  ...
  hands the decision to a human operator (farmer / agronomist). The
  approver resumes the pending request once a decision is made.
  `:flag-crop-health-concern` ALWAYS reaches escalation when the Governor
  is clean -- see `cerealops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `cattleops.operation`, cloud-itonami-isic-0141). This stub version
  ...
  (:require [cerealops.advisor :as advisor]
            [cerealops.governor :as governor]
            [cerealops.phase :as phase]))
...
(defn run-operation
  "Run one cereal-growing operation through the advisor -> governor ->
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- a `cerealops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
