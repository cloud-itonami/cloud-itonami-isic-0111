(ns cerealops.phase
  "Phase gate for the cereal-growing operations coordinator rollout
  (0->3 maturity). Mirrors `cattleops.phase` (cloud-itonami-isic-0141) in
  shape, adapted to this actor's simpler op set (no separate request-level
  :stake -- the request's :op IS the stake, since every op this actor may
  propose is already coordination-only).
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`cerealops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [cerealops.governor :as governor]))
